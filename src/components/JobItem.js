...
const JobItem = (props) => {
    ...
    const { id, type, created_at, company, location, title, company_logo, index } = props;
    ...
    return (
        <div className="job-item" index={index + 1} onClick={() => onItemClick(id)}>
            <div className="company-logo" ref={imageRef}>
                {isVisible && <Image src={company_logo} alt={company} width="100" height="100" />}
            </div>
            ...
            <div className="post-info">
                <div className="post-time">Posted {moment(new Date(created_at)).fromNow()}</div>
            </div>
        </div>
    );
};
